...
`src/lib/blog.ts`, and turned into pages by three independent pieces of code that each
re-derive the slug their own way. There is no CI, so the first signal is usually a human
clicking a dead link.

Read `context/content-pipeline.md` before diagnosing — the divergence table there is the
answer to most 404s.
...
2. **Is it a draft?** `draft: true` removes it from every `getAllPosts()` listing but
   still builds the page and still emits it into RSS. If the URL works and the listing
   does not, this is your answer.
...
4. **What slug does the link use?**
   [`getPostUrl()`](mex://function:455f6c97bf78e06c647c83f80b2d6235) strips
   `/\.(md|mdx)$/` from `filePath`. `[slug].astro`'s `getStaticPaths()` strips the literal
   string `'.md'`. For a `.mdx` file these disagree —
   `advanced.mdx` links to `/share/advanced` but builds `/share/advancedx`. **A `.mdx`
   post with a green build and a 404 is this bug.**
5. **Is the collection actually wired up?** A category needs entries in
   `[category]/index.astro`'s `getStaticPaths()` *and* its `categoryInfo` map,
   `[slug].astro`'s `Promise.all`, and `rss.xml.js`'s `Promise.all`. A collection that
   loads but has no route produces content the site cannot reach.
6. **Is it an ordering problem, not a missing-page problem?** `getAllPosts()` sorts by
   `publishedDate` descending; `getFeaturedPosts()` then takes the first three. A fourth
   featured post is not missing, it is fourth.
...
- **RSS is a third code path.** [`GET()`](mex://function:a23737293404e3e9b38254635fb9ad87)
  in `rss.xml.js` fans out and slugs independently of everything else, and it does not
  filter drafts. A feed problem is not necessarily a content problem.
...
- **`tests/blog.test.ts` only covers `getPostUrl`.** Passing tests say nothing about
  `getStaticPaths()`; that path has no coverage at all.
