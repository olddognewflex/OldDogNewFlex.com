...
2. Write the frontmatter. Required: `title`, `summary`, `publishedDate`. Optional:
   ...
   summary: "A short paragraph used in cards, meta description, and the RSS item."
...
- **`draft: true` does not hide the post from RSS.** `src/pages/rss.xml.js` calls
  `getCollection()` directly and never filters drafts. A draft is unlinked but still
  syndicated, and `[slug].astro` still builds a live page for it at its real URL. Treat
  `draft` as "not in listings", not as "private".
- **`featured` is capped at 3 by a hardcoded literal** in `getFeaturedPosts()`, not by
  `FEATURED_POSTS_LIMIT`. A fourth featured post silently never appears — the newest
  three win, since the list is already sorted by `publishedDate` descending.
...
- **A future `publishedDate` still publishes.** Nothing filters on date; the post just
  sorts to the top.
- **`summary` is user-visible in three places** — the card, the `<meta description>`, and
  the RSS item. Write it as prose, not as a slug.
...
- [ ] If `featured: true`, it appears on `/` — and check you have not pushed a fourth
      featured post off the list
- [ ] The hero image loads and is not multi-megabyte
- [ ] If still a draft, confirm you are comfortable that it is reachable by URL and in
      `/rss.xml`
