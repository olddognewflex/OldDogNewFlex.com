...
- **The flavour list is duplicated in six places** and they do not all agree in order:
  ...
  "fix" the theme list.
- **Cascade layers decide who wins, not specificity.** Tailwind v4 emits its utilities
  into cascade layers, and an **unlayered** rule beats any layered one *regardless of
  specificity*. The bare element selectors in `global.css` therefore silently outranked
  every `class="text-sm uppercase"` on a heading — section labels rendered at display
  size site-wide. They now live inside `@layer base` and must stay there. Moving a rule
  out of that block reintroduces the bug, and it will look like a Tailwind failure.
- **Long-form colour comes from `--tw-prose-*`, not from element rules.**
  `@tailwindcss/typography` ships light-mode defaults (near-black headings, body and
  links) and its `:where()` rules sit in a layer that outranks `@layer base`. Once the
  element defaults were layered, prose headings and links went dark-on-dark. The fix is
  the `.prose { --tw-prose-*: var(--color-*) }` block in `global.css` — set prose colour
  there, not by adding another `h2` rule.
- **Syntax highlighting follows the theme now.** `astro.config.mjs` uses
  `shikiConfig.themes: { light: "catppuccin-latte", dark: "catppuccin-macchiato" }` with
  `defaultColor: false`, so shiki emits `--shiki-light` / `--shiki-dark` variables and
  `global.css` picks between them — dark by default, light only under
  `:root[data-theme="latte"]`. The code-block *background* is deliberately
  `var(--color-mantle)` rather than shiki's own: shiki's dark background is the same
  colour as `--color-base` in macchiato, so blocks dissolved into the page.
